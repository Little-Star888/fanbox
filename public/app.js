...
// （独立弹窗版已无入口，逻辑抽成 mountReplay 供「本回合」面板底部嵌用；函数保留）
function openReplay() {
  ...
  ov.innerHTML =
    `<div class="replay-panel">
      <div class="replay-head"><span>会话回放 · ${tl.length} 次写入 · 跨 ${fmtDur(tl[tl.length - 1].ts - tl[0].ts)}</span><button class="replay-close ghost-btn">关闭 (Esc)</button></div>
      <div class="replay-mount"></div>
    </div>`;
  document.body.appendChild(ov);
  const rp = mountReplay(ov.querySelector('.replay-mount'), tl);
...
function mountReplay(host, tl) {
  ...
  const render = (frac) => {
    ...
    q('#replay-fill').style.width = (frac * 100) + '%';
    q('#replay-playhead').style.left = (frac * 100) + '%';
    q('#replay-now').textContent = lastIdx >= 0 ? tl[lastIdx].name : '—';
    q('#replay-count').textContent = `${done}/${tl.length}`;
    const recent = tl.slice(Math.max(0, lastIdx - 5), lastIdx + 1).reverse();
    q('#replay-list').innerHTML = recent.map((e, i) => `<div class="rl-row${i === 0 ? ' rl-now' : ''}"><span>${escapeHtml(e.name)}</span><span class="rl-t">${fmtClock(e.ts)}</span></div>`).join('');
  };
...
async function openRoundPanel() {
  ...
  const render = () => {
    ...
    body.innerHTML =
      ...
      `<div class="round-replay"><div class="round-replay-head">回放 · ${tl.length} 次写入${tl.length >= 2 ? ' · 跨 ' + fmtDur(tl[tl.length - 1].ts - tl[0].ts) : ''}</div><div class="round-replay-mount">${tl.length < 2 ? '<div class="round-empty">写入太少，没法回放</div>' : ''}</div></div>`;
